...
public class RegularBrowseFragment extends Fragment {

    private RegularBrowseViewModel regularBrowseViewModel;
    private RecyclerView ladiesRecycler;
    Query workersViewQuery;
    FirebaseRecyclerAdapter<RegularsModel, LadiesBrowseAdapter.RegularsViewModel> firebaseRecyclerAdapter;
    public View onCreateView(@NonNull LayoutInflater inflater,
                             ViewGroup container, Bundle savedInstanceState) {
        regularBrowseViewModel =
                ViewModelProviders.of(this).get(RegularBrowseViewModel.class);
        View root = inflater.inflate(R.layout.fragment_regular_browse, container, false);
        ...
        workersViewQuery= FirebaseDatabase.getInstance().getReference().child("Users").child(StaticKeys.commercials);
        ...
        FirebaseRecyclerOptions<RegularsModel> options=new FirebaseRecyclerOptions.Builder<RegularsModel>().setQuery(workersViewQuery,RegularsModel.class)
                .build();
        firebaseRecyclerAdapter=new FirebaseRecyclerAdapter<RegularsModel, LadiesBrowseAdapter.RegularsViewModel>
                (options) {
            @Override
            protected void onBindViewHolder(@NonNull LadiesBrowseAdapter.RegularsViewModel holder, int position, @NonNull final RegularsModel model) {
            ...
            @NonNull
            @Override
            public LadiesBrowseAdapter.RegularsViewModel onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
                View v= LayoutInflater.from(parent.getContext()).inflate(R.layout.ladies_item_layout,parent,false);
                return  new LadiesBrowseAdapter.RegularsViewModel(v);
